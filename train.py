"""
Autoresearch pretraining script. Single-CPU, single-file.
...
class CausalSelfAttention(nn.Module):
    ...
    def forward(self, x, ve, cos_sin, window_size):
        ...
        # Use PyTorch's scaled_dot_product_attention (works on CPU)
...
# ---------------------------------------------------------------------------
# Optimizer (MuonAdamW, CPU-compatible)
# ---------------------------------------------------------------------------
...
HEAD_DIM = 64           # target head dimension for attention (smaller for CPU)
WINDOW_PATTERN = "SSSL" # sliding window pattern: L=full, S=half context

# Optimization
TOTAL_BATCH_SIZE = 2**14 # ~16K tokens per optimizer step (reduced for CPU)
...
# Model size
DEPTH = 4               # number of transformer layers (reduced for CPU)
DEVICE_BATCH_SIZE = 8   # per-device batch size (reduced for CPU)
...
if __name__ == "__main__":
    ...
    while True:
        ...
        # Time's up — but only stop after warmup steps so we don't count initial overhead
